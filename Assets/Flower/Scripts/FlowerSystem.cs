...
namespace Flower{
    ...
    public class FlowerSystem : MonoBehaviour
    {
        ...
        {
            get { return isTextListCompleted && !hasActiveButtons; }
        }
        public string text { get { return msgText.ToString(); } }
        public bool isWaitingForNext { get { return isWaitingForNextToGo; } }
        public static float textSpeed = SettingsManager.TextSpeed;
        public static float dialogAlpha = SettingsManager.DialogAlpha;
        ...
        private bool hasActiveButtons = false;
        ...
        public void StopAndReset()
        {
            ...
            // 移除所有圖片和UI元素
            foreach (var kvp in sceneObjectMap.ToList())
            {
                if (kvp.Value != null)
                {
                    Destroy(kvp.Value);
                }
                sceneObjectMap.Remove(kvp.Key);
            }
        ...
        // 重置預設路徑映射的輔助方法
        private void ResetPrefabPathMap()
        {
            prefabPathMap.Clear();
            SetPrefabPath("image", "DefaultImagePrefab");
            SetPrefabPath("audio", "DefaultAudioPrefab");
        ...
        protected virtual void OnResetCompleted()
        {
       ...
       public void SetupDialog(string resourcePath="DefaultDialogPrefab", bool applyDefaultTextUpdateEvent=true, bool applyDefaultTextSplitting=false){
        ...
        public void SetupUIStage(string key, string resourcePath, int sortLayer, Camera renderCamera)
        {
            try
            {
                // 从指定路径加载预制件资源
                ...
                // 在场景中创建该预制件的实例，并命名为 "_UIStage_" + key
                var uiStage = CreateAsSceneObject("_UIStage_" + key, UIStagePrefab, Vector3.zero, false);

                // 获取实例中的 Canvas 组件
                var canvas = uiStage.GetComponent<Canvas>();

                if (canvas != null)
                {
                    // 设置 Canvas 的 sortingOrder，决定 UI 的渲染顺序
                    canvas.sortingOrder = sortLayer;

                    // 设置 Canvas 渲染模式为 Screen Space - Camera
                ...
                {
                    Debug.LogWarning("Canvas component not found on the prefab.");
                }
            }
            catch (Exception e)
            {
